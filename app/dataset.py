...
def generate_rows(seed: int = 20260426, size: int = 2400) -> list[dict[str, float | int | str]]:
    ...
    segments = [
        ("new_high_intent", 0.18, 0.14),
        ("loyal_discount_insensitive", 0.42, -0.02),
        ("dormant_price_sensitive", 0.08, 0.09),
        ("casual_mid_value", 0.15, 0.03),
    ]

    for index in range(size):
        segment_name, base_rate, treatment_effect = segments[index % len(segments)]
